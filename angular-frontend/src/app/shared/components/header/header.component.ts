...
@Component({
  selector: 'app-header',
  standalone: true,
  imports: [RouterLink, RouterLinkActive, NgIf, NgFor, NgClass, FormsModule, BreadcrumbComponent],
  styles: [`
    ...
    .pinkbar .nav{
      @apply container px-3 flex items-center gap-2;
      overflow: visible;
    }
    @media (max-width: 768px){
      .pinkbar .nav{ overflow-x:auto; overflow-y:visible; }
    }
    ...
    .nav-item--cat{ background:#fb7185; color:#fff; box-shadow: 0 6px 18px rgba(244,63,94,.15); }
    ...
    .menu{
      @apply absolute left-0 top-full mt-2 w-[min(1100px,calc(100vw-32px))] bg-white rounded-2xl shadow-2xl border border-rose-100 overflow-hidden z-[85];
    }
    ...
    .menu-item-left.is-active{ background:#ffe7eb; border-left: 3px solid #e11d48; }
...
<header>
  ...
  <div class="row mid">
    ...
    <div class="acts">
      ...
      <!-- Wishlist -->
      <a routerLink="/wishlist" class="act relative" title="Yêu thích">
        <span class="ic relative">
          <img src="assets/icon/heart.png" class="w-4 h-4" alt="wishlist"/>
          <span *ngIf="wishlistCount()>0" class="cart-badge">{{ wishlistCount() }}</span>
        </span>
        <span class="txt"><b>Yêu thích</b></span>
      </a>
    </div>
  </div>
  ...
  <div class="pinkbar">
    <nav class="nav">
      ...
      <div class="relative">
        <button class="nav-item nav-item--cat"
                aria-haspopup="menu"
      ...
      <a routerLink="/flash" routerLinkActive="bg-white/10" class="nav-item">Flash sale <span class="hot-pill">HOT</span></a>
...
export class HeaderComponent {
  ...
  private catStore = inject(CategoryStore);
  ...
  constructor() {
    this.auth.user$.pipe(takeUntilDestroyed()).subscribe(u => {
      this.user.set(u);
      if (u) {
        this.wishlist.loadIds().subscribe();     // ⭐ nạp ID + count
      } else {
        this.wishlist.likedIds.set(new Set());
        this.wishlist.count.set(0);
      }
    });

    // F5: có token nhưng chưa có snapshot user -> fetch me rồi nạp ids
    if (this.auth.token && !this.auth.userSnapshot()) {
      this.auth.fetchMe().pipe(takeUntilDestroyed()).subscribe({
        next: () => this.wishlist.loadIds().subscribe(),
        error: () => this.auth.logout(false)
      });
    }

    this.catStore.ensure();

    const setFirst = () => {
      const ps = this.parents();
      if (ps.length && !this.hoverId()) this.hoverId.set(ps[0].id);
    };
    queueMicrotask(setFirst);
    setTimeout(setFirst, 0);
  ...
  toggleMenu(ev?: Event){ ev?.preventDefault(); this.open.set(!this.open()); }
  ...
  private updateCartCount(){
    try{
      ...
      let total = 0;
      if (obj && typeof obj.count === 'number') total = obj.count;
      else if (Array.isArray(obj)) total = obj.reduce((s:number,it:any)=> s + Number(it?.qty || 1), 0);
  ...
  logout(){
    ...
    // clear wishlist state để UI sạch sẽ
    this.wishlist.likedIds.set(new Set());
    this.wishlist.count.set(0);
