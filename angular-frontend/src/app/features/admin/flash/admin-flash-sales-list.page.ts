...
@Component({
  ...
  // Tailwind là chủ đạo, styles chỉ để fallback rất nhẹ
  styles: [`
  ...
  <div class="p-4">
    ...
    <div class="flex items-center gap-3 mb-4">
      ...
      <a routerLink="/admin/flash-sales/new"
         class="h-11 px-4 rounded-xl border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100 inline-flex items-center font-medium">
        + Tạo flash sale
      </a>
    </div>
      ...
      <table class="min-w-[860px] w-full text-[15px]">
        <thead class="bg-slate-50 text-slate-600">
          <tr>
            ...
            <th class="sticky-th text-right font-semibold px-3 py-3 w-[80px]">SP</th>
            <th class="sticky-th text-center font-semibold px-3 py-3 w-[110px]">Trạng thái</th>
            <th class="sticky-th px-3 py-3 w-[220px]"></th>
          </tr>
        </thead>
        ...
        <tbody>
          <tr *ngFor="let s of list(); let i = index"
            ...
            <td class="px-3 py-3">
              <div class="font-medium text-slate-900 leading-5 line-clamp-2">
                {{ s.name }}
              </div>
            </td>

            <td class="px-3 py-3">
              <div class="max-w-[200px] truncate text-slate-700">{{ s.slug }}</div>
            </td>
            ...
            <td class="px-3 py-3 text-slate-700">
              {{ s.startAt | date:'yyyy-MM-dd HH:mm' }}
            </td>
            <td class="px-3 py-3 text-slate-700">
              {{ s.endAt   | date:'yyyy-MM-dd HH:mm' }}
            </td>
            ...
            <td class="px-3 py-3 text-center">
              <span
                class="inline-flex items-center h-7 px-2.5 rounded-lg text-[13px] font-medium"
                [class.bg-emerald-50]="s.active" [class.text-emerald-700]="s.active"
                [class.bg-rose-50]="!s.active" [class.text-rose-700]="!s.active">
            ...
            <td class="px-3 py-3">
              <div class="flex justify-end gap-2">
                <a class="h-9 px-3 rounded-lg border border-slate-200 hover:bg-slate-50 inline-flex items-center"
                   [routerLink]="['/admin/flash-sales', s.id, 'edit']">Sửa</a>
                <button class="h-9 px-3 rounded-lg border border-slate-200 hover:bg-slate-50"
                        type="button" (click)="toggle(s)">{{ s.active ? 'Tắt' : 'Bật' }}</button>
                <button class="h-9 px-3 rounded-lg border border-rose-200 text-rose-700 hover:bg-rose-50"
                        type="button" (click)="del(s)">Xoá</button>
              </div>
            </td>
          </tr>
        </tbody>
...
export default class AdminFlashSalesListPageComponent {
  ...
  del(s: FlashSaleAdminListItem){
    if (!confirm(`Xoá flash sale "${s.name}"?`)) return; // bỏ dấu '\'
